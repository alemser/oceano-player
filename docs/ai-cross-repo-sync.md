...
| Item | Type | Notes |
|------|------|--------|
| `GET /api/airplay/transport-capabilities` | **Semantic** | Now uses live DACP context from shairport metadata pipe (`acre`, `daid`, `clip`) in `oceano-web`; returns deterministic readiness (`ready`, `no_airplay_session`, `missing_dacp_context`, `session_stale`, `amp_off`). |
