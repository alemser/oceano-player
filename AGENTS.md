...
- After **20 minutes** without effective playback, the clock / weather idle screen appears. Until then, the Now Playing view can stay on the last frame (dimmed standby when `chromeIdle` applies; short inter-track gaps on physical sources do not block the clock timer)
...
- **Documentation stays in sync**: when architecture/workflows change, update README/Codex/install help in the same change set.
...
- **AGENTS.md** — keep Architecture, Deployment, and What NOT to do current. If a section describes something that no longer exists, update or remove it.
